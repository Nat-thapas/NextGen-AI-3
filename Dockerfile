...
RUN mkdir -p /var/run/nextgen-ai-3
...
COPY --from=prod-deps --chown=nonroot:nonroot /app/files ./files
COPY --from=prod-deps --chown=nonroot:nonroot /var/run/nextgen-ai-3 /var/run/nextgen-ai-3
