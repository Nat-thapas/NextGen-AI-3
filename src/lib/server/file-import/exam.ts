	...
	for (const sheet of sheets) {
		...
		for (const [rowNumber, row] of data.entries()) {
			...
			if (row[0].toLowerCase() === '$question') {
				...
				const textLengthLimit = questionType === questionTypes.text ? Number(row[6] ?? 1000) : null;
				const textCorrect =
			...
			if (row[0].toLowerCase() === '$choice') {
				if (question === undefined) {
					throw Error(`Sheet '${name}' Cell A${rowNumber + 1} Choice is defined before a question`);
				}
